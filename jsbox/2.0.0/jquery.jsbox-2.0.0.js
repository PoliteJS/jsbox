...
function initDOM(box) {
    ...
    box.$el.append(box.$tests).append(box.$editor).append(box.$sandbox).append(box.$exception).append(box.$console).append(box.$runBtn).append(box.$resetBtn).append(box.$overlay);
}

function disposeDOM(box) {
    box.$runBtn.off();
    box.$resetBtn.off();
    box.$el.empty().removeClass("jsbox-widget").html(box._originalHTML);
}
